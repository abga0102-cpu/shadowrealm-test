/* Combat Animation V167
   Consolidated locomotion + continuous articulated walk + weapon choreography.
   Replaces V166 with seam-free leg motion and robust enemy morphology detection.
...
(function(){
  ...
  function disableLegs(unit,img){
    unit.classList.remove("srWalkPseudo167");
  ...
  function articulate(unit,img,phase,size,dir,flip,profile){
    ...
    const wave=Math.sin(q);
    const s=dir<0?-1:1;
    const step=size*profile.step,lift=size*profile.lift;
    const leftSwing=smooth01(Math.max(0,wave));
    const rightSwing=smooth01(Math.max(0,-wave));
    const lx=wave*step*0.56*s;
    const rx=-wave*step*0.56*s;
    const ly=-leftSwing*lift;
    const ry=-rightSwing*lift;
    const lr=wave*2.2*s;
    const rr=-wave*2.2*s;
    unit.classList.add("srWalkPseudo167");
  ...
  drawArena=function(){
    ...
    try{
      ...
      rendered.forEach(e=>{
        ...
        if(active&&gait.moving){
          ...
          const morph=profileForEnemy(e,img);if(morph)articulate(unit,img,gait.phase,size,gait.dir||-1,true,morph);else disableLegs(unit,img);
    ...
    }catch(err){if(typeof window!=="undefined")window.__srWalkV167Error=String(err&&err.message||err);}
  };

  const style=document.createElement("style");
  style.id="combat-animation-v167-style";
  style.textContent=[
    ...
    ".srWalkPseudo167::before,.srWalkPseudo167::after{content:'';position:absolute;inset:0;pointer-events:none;z-index:1;background-image:var(--sr-sprite);background-size:contain;background-position:center;background-repeat:no-repeat;transform-origin:50% 92%;will-change:transform}",
    ".srWalkPseudo167::before{clip-path:polygon(0 var(--sr-split-top),var(--sr-left-end) var(--sr-split-top),var(--sr-left-end) 100%,0 100%);transform:var(--sr-flip) translate(var(--sr-lx),var(--sr-ly)) rotate(var(--sr-lr))}",
    ".srWalkPseudo167::after{clip-path:polygon(var(--sr-right-start) var(--sr-split-top),100% var(--sr-split-top),100% 100%,var(--sr-right-start) 100%);transform:var(--sr-flip) translate(var(--sr-rx),var(--sr-ry)) rotate(var(--sr-rr))}",
    "@media (prefers-reduced-motion:reduce){.unit>img,.srWalkPseudo167::before,.srWalkPseudo167::after{will-change:auto}}"
  ].join("\n");
