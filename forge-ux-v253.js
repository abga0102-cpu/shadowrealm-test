/* SHADOWREACH · Forge UX V253
   ...
   - AUTO can forge 1/3/5/10/20 pieces per cycle from the Forge filter modal.
   - Batch choice persists in S.forge.autoBatch (optional/backward compatible).
   - Recycled items still use the existing economy result from forgeSummon.
*/
(function(){
'use strict';
...
function ensureRoot(){var r=document.getElementById(ROOT);if(r)return r;r=document.createElement('div');r.id=ROOT;r.setAttribute('aria-live','polite');r.style.cssText='position:fixed;z-index:10920;left:50%;top:75%;width:0;height:0;pointer-events:none;';document.body.appendChild(r);return r;}
function anchor(){var root=ensureRoot();var f=document.querySelector('[data-act="forgeFilter"]');var card=f&&f.closest('.card');if(card){var rc=card.getBoundingClientRect(),fr=f.getBoundingClientRect();var x=Math.max(78,Math.min(innerWidth-78,rc.left+rc.width/2));var y=Math.max(rc.top+84,Math.min(rc.bottom-100,fr.top-72));root.style.left=x+'px';root.style.top=y+'px';}else{root.style.left='50%';root.style.top='76%';}}
...
forgeSummon=function(){var out=nativeForgeSummon.apply(this,arguments);try{enqueue(out);}catch(e){console.warn('forge visual V253 skipped',e);}return out;};try{window.forgeSummon=forgeSummon;}catch(_){}

/* AUTO runtime authority: V199 owns the comparison pause, V253 only changes how many
   forge attempts happen in one AUTO cycle. Kept drops still pause the cycle exactly as before. */
var paused=function(){try{return !!(window.__srAutoForgePausedForCompareV199&&window.__srAutoForgePausedForCompareV199());}catch(_){return false;}};
scheduleAutoForge=function(delay){if(paused())return;if(typeof autoForgeTimer!=='undefined'&&autoForgeTimer!==null)return;autoForgeTimer=setTimeout(function(){autoForgeTimer=null;if(!S.forge.autoForge||paused())return;var rearm=true;try{var cost=forgeCost(S.forge.level),want=batch(),aff=Math.max(0,Math.floor(Number(S.minerai||0)/Math.max(1,cost))),n=Math.max(0,Math.min(want,aff));if(n>0){var res=forgeSummon(n)||[],kept=res.filter(function(r){return !!(r&&!r.recycled&&r.id);});if(kept.length){kept.forEach(function(r){r.__autoForgeCompareV199=true;});rearm=false;if(typeof showForgeResult==='function')showForgeResult(kept);}}}catch(err){console.error('auto-forge V253 tick failed',err);}finally{if(rearm&&S.forge.autoForge&&!paused())scheduleAutoForge(1500);}},Math.max(0,delay==null?1500:delay));};
try{if(typeof autoForgeTimer!=='undefined'&&autoForgeTimer!==null){clearTimeout(autoForgeTimer);autoForgeTimer=null;}if(S.forge.autoForge)scheduleAutoForge(120);}catch(_){}

if(typeof ACT!=='undefined'&&ACT){ACT.autoForgeBatch253=function(a){var n=Math.floor(Number(a)||1);persistBatch(n);try{if(typeof autoForgeTimer!=='undefined'&&autoForgeTimer!==null){clearTimeout(autoForgeTimer);autoForgeTimer=null;}if(S.forge.autoForge&&!paused())scheduleAutoForge(180);}catch(_){};if(nativeFilter)showForgeFilterPicker();};}
function injectBatch(){var toggle=document.querySelector('[data-act="forgeFilterToggle"]');if(!toggle)return;var row=toggle.parentElement;if(!row||document.getElementById('srAutoBatch253'))return;var box=document.createElement('div');box.id='srAutoBatch253';box.innerHTML='<div class="b small">Auto-Forge simultanée</div><div class="mute tiny mt2">Nombre de pièces forgées à chaque cycle AUTO.</div><div class="srBatchBtns253">'+BATCHES.map(function(n){return '<button type="button" class="srBatch253 '+(batch()===n?'on':'')+'" data-act="autoForgeBatch253" data-arg="'+n+'">×'+n+'</button>';}).join('')+'</div><div class="mute tiny mt3">Le coût reste normal par pièce. Si le minerai manque, le jeu forge seulement la quantité possible.</div>';row.insertAdjacentElement('afterend',box);}
...
#'+ROOT+' .srForgeMeta253{font-size:8px;font-weight:800;color:#97a8c1;margin:4px 4px 7px}.srForgeMeta253 b{color:var(--r)}\
