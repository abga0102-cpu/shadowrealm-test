...
- the permanent navigation taxonomy is now also centralized in `bottom-nav-layout-v183.js`: the legacy route IDs remain intact, but the visible four-tab model is Home / Equipment / Progression / Menu, Progression/Menu hubs add only missing valid routes, redundant permanent Home Development/Challenges/Menu shortcuts are suppressed, and the retired Rebirth route is not reintroduced on Home or in Progression; no new runtime layer is introduced;
- `premium-ui-v209.js` owns one visual CTA hierarchy across legacy button classes—gold primary, navy/steel secondary, red danger, muted disabled—so domain jewel colors no longer imply competing action semantics;
