...
function migrateItem(it){if(!it||!it.rarity)return;var targetVersion=372;if(Number(it.powerCurveVersion)>=targetVersion)return;var b=EQUIP_BASE[it.rarity];if(!b){it.powerCurveVersion=targetVersion;return;}var q=Number(it.statQuality);if(!(q>0&&q<=1))q=qroll(it.rarity,seeded(hash((it.id||'')+'|'+it.rarity+'|'+it.slot)));var star=equipStar(),x=equipStats(it.slot,it.rarity,q,star),oldBD=Number(it.baseDamage)||0,oldBH=Number(it.baseHp)||0,oldD=Number(it.damage)||0,oldH=Number(it.hp)||0;var df=oldBD>0?Math.max(1,oldD/oldBD):1,hf=oldBH>0?Math.max(1,oldH/oldBH):1;it.baseDamage=x.d;it.baseHp=x.h;it.damage=x.d*df;it.hp=x.h*hf;it.originalPower=it.baseDamage+it.baseHp;it.power=it.damage+it.hp;it.statQuality=Math.round(q*10000)/10000;it.powerCurveVersion=targetVersion;}
