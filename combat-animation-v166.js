/* Combat Animation V166
   Consolidated locomotion + articulated walk + weapon choreography.
   Replaces V164/V165. No clone nodes, no DOM prototype monkey-patching.
...
(function(){
  ...
  function disableLegs(unit,img){
    unit.classList.remove("srWalkPseudo166");
  ...
  function articulate(unit,img,phase,size,dir,flip,profile){
    ...
    const half=(phase*2)%1,leftStance=phase<0.5,prog=smooth01(half);
    const step=size*profile.step,lift=size*profile.lift,s=dir<0?-1:1;
    const stanceX=(0.5-half)*step*0.95*s;
    const swingX=(-0.5+prog)*step*1.20*s;
    const swingY=-Math.sin(prog*Math.PI)*lift;
    const swingRot=Math.sin((prog-0.5)*Math.PI)*2.2*s;
    const lx=leftStance?stanceX:swingX, rx=leftStance?swingX:stanceX;
    const ly=leftStance?0:swingY, ry=leftStance?swingY:0;
    const lr=leftStance?0:swingRot, rr=leftStance?swingRot:0;
    unit.classList.add("srWalkPseudo166");
    ...
    unit.style.setProperty("--sr-lx",px(lx));unit.style.setProperty("--sr-ly",px(ly));unit.style.setProperty("--sr-lr",lr.toFixed(2)+"deg");
    unit.style.setProperty("--sr-rx",px(rx));unit.style.setProperty("--sr-ry",px(ry));unit.style.setProperty("--sr-rr",rr.toFixed(2)+"deg");
  ...
  drawArena=function(){
    ...
    try{
      ...
      rendered.forEach(e=>{
        ...
        if(active&&gait.moving){
          ...
          const morph=profileForEnemy(e);if(morph)articulate(unit,img,gait.phase,size,gait.dir||-1,true,morph);else disableLegs(unit,img);
    ...
    }catch(err){if(typeof window!=="undefined")window.__srWalkV166Error=String(err&&err.message||err);}
  };

  const style=document.createElement("style");
  style.id="combat-animation-v166-style";
  style.textContent=[
    ...
    ".srWalkPseudo166::before,.srWalkPseudo166::after{content:'';position:absolute;inset:0;pointer-events:none;z-index:1;background-image:var(--sr-sprite);background-size:contain;background-position:center;background-repeat:no-repeat;transform-origin:50% 92%;will-change:transform}",
    ".srWalkPseudo166::before{clip-path:polygon(0 var(--sr-split-top),var(--sr-left-end) var(--sr-split-top),var(--sr-left-end) 100%,0 100%);transform:var(--sr-flip) translate(var(--sr-lx),var(--sr-ly)) rotate(var(--sr-lr))}",
    ".srWalkPseudo166::after{clip-path:polygon(var(--sr-right-start) var(--sr-split-top),100% var(--sr-split-top),100% 100%,var(--sr-right-start) 100%);transform:var(--sr-flip) translate(var(--sr-rx),var(--sr-ry)) rotate(var(--sr-rr))}",
    "@media (prefers-reduced-motion:reduce){.unit>img,.srWalkPseudo166::before,.srWalkPseudo166::after{will-change:auto}}"
  ].join("\n");
