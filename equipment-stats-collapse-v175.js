/* SHADOWREACH · Equipment combat stats collapse V175
...
(function(){
'use strict';
if(window.__srEquipmentStatsCollapseV175)return;
window.__srEquipmentStatsCollapseV175=true;

var KEY='sr:equipmentCombatStatsExpanded:v175';
function pref(){
...
function apply(){
  ...
  if(!title||title.getAttribute('data-stats-v175')==='1')return;
  var card=title.nextElementSibling;
  if(!card||!card.classList.contains('card'))return;
  title.setAttribute('data-stats-v175','1');
  ...
  action.setAttribute('data-stats-toggle-v175','1');
  action.style.cursor='pointer';
  action.style.flex='0 0 auto';
  action.style.color='var(--cyanLit,#8FEFF4)';
  action.style.borderColor='var(--cyan,#3FCFD6)';
  title.appendChild(action);
  function sync(open){
    card.style.display=open?'':'none';
    action.textContent=open?'Masquer ▲':'Afficher ▼';
