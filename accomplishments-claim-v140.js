/* SHADOWREACH · Accomplishments claim reliability v140 · Fusion milestones V208
   V314: campaign milestones extend to 40-10; retired Rebirth/PR rewards are inert. */
(function(){
'use strict';
...
var REWARDS={
 ...
 floor25:{essence:250},
 floor50:{minerai:2000,gold:5000},
 floor75:{eclat:500,merge:{COMMUN:30}},
 floor100:{eclat:500,essence:500,merge:{COMMUN:30}},
 floor150:{eclat:750,essence:750,merge:{RARE:15}},
 floor200:{eclat:1000,essence:1000,merge:{RARE:20}},
 floor250:{eclat:1250,essence:1250,merge:{EPIQUE:10}},
 floor300:{eclat:1500,essence:1500,merge:{EPIQUE:15}},
 floor350:{eclat:2000,essence:2000,merge:{MYTHIQUE:10}},
 floor400:{eclat:2500,essence:2500,merge:{MYTHIQUE:20},universal:1}
};
...
var NEED={
 ...
 floor25:function(){return Number(S.recordFloor)>=25;},floor50:function(){return bossClear(50);},floor75:function(){return Number(S.recordFloor)>=75;},floor100:function(){return bossClear(100);},floor150:function(){return bossClear(150);},floor200:function(){return bossClear(200);},floor250:function(){return bossClear(250);},floor300:function(){return bossClear(300);},floor350:function(){return bossClear(350);},floor400:function(){return bossClear(400);}
};
...
function claim(id,choice){if(typeof S==='undefined'||!S||!REWARDS[id]||!NEED[id]||!NEED[id]())return false;var a=ensure(S),r=REWARDS[id];if(a.claimed[id])return false;if(r.choice&&choice!=='eclat'&&choice!=='essence')return false;if(typeof update==='function'){update(function(s){var x=ensure(s);if(x.claimed[id])return;var st=s.sanctuary||{};x.fusionCount=Math.max(Number(x.fusionCount)||0,Number(st.mergeCrafts)||0,Number(st.fusions)||0);grant(s,r,choice);x.claimed[id]=true;if(choice)x.choices[id]=choice;});}else{a.fusionCount=Math.max(Number(a.fusionCount)||0,Number(S.sanctuary&&S.sanctuary.mergeCrafts)||0,Number(S.sanctuary&&S.sanctuary.fusions)||0);grant(S,r,choice);a.claimed[id]=true;if(choice)a.choices[id]=choice;try{dirty=true;if(typeof saveNow==='function')saveNow();}catch(_){}}try{if(typeof toast==='function')toast('Récompense reçue !',true);}catch(_){}try{window.dispatchEvent(new CustomEvent('sr:accomplishmentclaimed',{detail:{id:id,choice:choice||''}}));}catch(_){}try{if(typeof ACT!=='undefined'&&ACT&&typeof ACT.accomplishments==='function')ACT.accomplishments();}catch(_){}return true;}
...
function markClaimedButton(b){try{if(!b||!b.isConnected)return;var s=document.createElement('span');s.className='pill';s.style.cssText='color:var(--greenLit);border-color:#3FB950;flex:0 0 auto';s.textContent='Récupéré';var host=b.parentNode;if(host&&host.getAttribute&&host.getAttribute('data-ach-choice'))return;b.replaceWith(s);}catch(_){} }
compensateLegacyForge();setTimeout(compensateLegacyForge,300);setTimeout(compensateLegacyForge,1200);
document.addEventListener('click',function(e){var b=e.target&&e.target.closest?e.target.closest('.srAch139 [data-ach]'):null;if(!b)return;e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();var ok=claim(String(b.getAttribute('data-ach')||''),String(b.getAttribute('data-ach-choice')||''));if(ok)markClaimedButton(b);},true);
})();