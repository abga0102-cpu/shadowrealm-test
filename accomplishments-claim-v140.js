/* SHADOWREACH · Accomplishments claim reliability v140 · Forge balance V200
   Reliable reward claiming plus one-time compensation for Forge rewards
   upgraded after players may already have claimed older values. */
(function(){
'use strict';
...
function claim(id,choice){if(typeof S==='undefined'||!S||!REWARDS[id]||!NEED[id]||!NEED[id]())return false;var a=ensure(S),r=REWARDS[id];if(a.claimed[id])return false;if(r.choice&&choice!=='eclat'&&choice!=='essence')return false;if(typeof update==='function'){update(function(s){var x=ensure(s);if(x.claimed[id])return;grant(s,r,choice);x.claimed[id]=true;if(choice)x.choices[id]=choice;});}else{grant(S,r,choice);a.claimed[id]=true;if(choice)a.choices[id]=choice;try{dirty=true;if(typeof saveNow==='function')saveNow();}catch(_){}}try{if(typeof toast==='function')toast('Récompense reçue !',true);}catch(_){}try{if(typeof ACT!=='undefined'&&ACT&&typeof ACT.accomplishments==='function')ACT.accomplishments();}catch(_){}return true;}
function compensateLegacyForge(){
 try{
  if(typeof S==='undefined'||!S)return;
  var a=ensure(S);if(a.forgeRewardBalanceV200Processed)return;
  var paid=[];
  function addMerge(s,r,q){var x=ensure(s);x.mergePieces[r]=(Number(x.mergePieces[r])||0)+q;}
  function migrate(s){var x=ensure(s);if(x.forgeRewardBalanceV200Processed)return;
   if(x.claimed.forge20){addMerge(s,'PEU_COMMUN',15);paid.push('Forge 20');}
   if(x.claimed.forge30){addMerge(s,'PEU_COMMUN',20);paid.push('Forge 30');}
   if(x.claimed.forge35){addMerge(s,'RARE',25);if(s.raids&&s.raids.minerai)s.raids.minerai.keys=(Number(s.raids.minerai.keys)||0)+1;paid.push('Forge 35');}
   if(x.claimed.forge40){addMerge(s,'RARE',20);paid.push('Forge 40');}
   if(x.claimed.forge50){addMerge(s,'EPIQUE',25);paid.push('Forge 50');}
   x.forgeRewardBalanceV200Processed=true;x.forgeRewardBalanceV200Paid=paid.slice();x.forgeRewardBalanceV200At=Date.now();
  }
  if(typeof update==='function')update(migrate);else{migrate(S);try{dirty=true;if(typeof saveNow==='function')saveNow();}catch(_){}}
  if(paid.length){try{if(typeof toast==='function')toast('Compensation Forge reçue : '+paid.join(', '),true);}catch(_){}}
 }catch(_){}
}
compensateLegacyForge();setTimeout(compensateLegacyForge,300);setTimeout(compensateLegacyForge,1200);
document.addEventListener('click',function(e){var b=e.target&&e.target.closest?e.target.closest('.srAch139 [data-ach]'):null;if(!b)return;e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();claim(String(b.getAttribute('data-ach')||''),String(b.getAttribute('data-ach-choice')||''));},true);
})();