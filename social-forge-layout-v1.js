/* SHADOWREACH HOME FRAME LAYOUT V12
   Home owns its chat preview: the preview may only render inside #arena, never
   at app root. Rebirth and Challenges are compact left-side actions; Development
   remains in the permanent bottom navigation. */
(()=>{
"use strict";
...
/* Compact left rail: Rebirth, then Challenges. Development stays in bottom nav. */
...
#app.srHomeFullArena .worldRebirth{top:calc(var(--srHudH) + 5px)!important;bottom:auto!important}
#app.srHomeFullArena .worldDefis{display:flex!important;top:calc(var(--srHudH) + 43px)!important;bottom:auto!important}
...
/* Status moves higher into the free centre area. */
...
/* Home ownership rule: no root-level chat button/preview may ever be visible. */
#app.srHomeFullArena>#srChatBtn,#app.srHomeFullArena>#srChatPreview{display:none!important}

/* Chat has its own visual lane inside the arena only. */
...
@media(max-width:370px){
 ...
 #app.srHomeFullArena .worldDefis{top:calc(var(--srHudH) + 41px)!important}
...
@media(max-height:720px){
 ...
 #app.srHomeFullArena .worldRebirth{top:calc(var(--srHudH) + 4px)!important}
 #app.srHomeFullArena .worldDefis{top:calc(var(--srHudH) + 39px)!important}
...
function sync(){
 ...
 requestAnimationFrame(()=>{
  ...
  const btn=document.getElementById("srChatBtn");
  ...
  if(home){
   ...
   if(!p)p=makePreview();
   if(p.parentElement!==arena)arena.appendChild(p);
   updatePreview(p);
  }else{
   /* Never park the preview at app root: that was the source of the huge BOT text. */
   if(p)p.remove();
