...
.sanctMergePiece.srSanctReady181{box-shadow:0 0 0 1px color-mix(in srgb,var(--srReady,#8FEFF4) 70%,transparent),0 0 13px color-mix(in srgb,var(--srReady,#8FEFF4) 42%,transparent)!important;animation:srSanctReadyGlow181 1.45s ease-in-out infinite;animation-delay:var(--srReadyDelay,0ms)}\
.sanctMergePiece.srSanctReady181>div:first-child{transform-origin:50% 52%;will-change:transform,filter;animation:srSanctReadyOrb181 1.45s cubic-bezier(.25,.7,.3,1) infinite;animation-delay:var(--srReadyDelay,0ms);filter:drop-shadow(0 0 5px color-mix(in srgb,var(--srReady,#8FEFF4) 58%,transparent))}\
html.srSanctDragActive181 .sanctMergePiece.srSanctReady181,html.srSanctDragActive181 .sanctMergePiece.srSanctReady181>div:first-child{animation:none!important}\
...
@keyframes srSanctReadyGlow181{0%,100%{filter:brightness(1)}46%{filter:brightness(1.08)}72%{filter:brightness(1.16)}84%{filter:brightness(1.04)}}\
@keyframes srSanctReadyOrb181{0%,58%,100%{transform:translate3d(0,0,0) rotate(0deg) scale(1)}64%{transform:translate3d(-1.5px,0,0) rotate(-2.2deg) scale(1.035)}69%{transform:translate3d(1.5px,-.5px,0) rotate(2.2deg) scale(1.04)}74%{transform:translate3d(-1px,.5px,0) rotate(-1.5deg) scale(1.035)}79%{transform:translate3d(1px,0,0) rotate(1.2deg) scale(1.025)}84%{transform:translate3d(0,0,0) rotate(0deg) scale(1)}}\
...
@media(prefers-reduced-motion:reduce){.sanctMergeCell.srCompat181{animation:none}.sanctMergePiece.srSanctReady181,.sanctMergePiece.srSanctReady181>div:first-child{animation:none!important}.sanctMergePiece.srSanctReady181{box-shadow:0 0 0 1px var(--srReady,#8FEFF4),0 0 10px color-mix(in srgb,var(--srReady,#8FEFF4) 38%,transparent)!important}.srSanctLocalFx181{display:none}}';document.head.appendChild(s);}
function colorFor(r){try{return (SANCT_MERGE_COLOR&&SANCT_MERGE_COLOR[r])||'#8FEFF4';}catch(_){return '#8FEFF4';}}
function clearReadyPairs(){document.querySelectorAll('.sanctMergePiece.srSanctReady181').forEach(function(el){el.classList.remove('srSanctReady181');el.style.removeProperty('--srReady');el.style.removeProperty('--srReadyDelay');});}
function refreshReadyPairs(){
  clearReadyPairs();
  var groups={};
  document.querySelectorAll('.sanctMergePiece[data-sanct-slot][data-sanct-rarity]').forEach(function(el){
    var r=el.dataset.sanctRarity||'';
    if(!r)return;
    var next=null;try{next=typeof sanctMergeNext==='function'?sanctMergeNext(r):null;}catch(_){}
    if(!next)return;
    (groups[r]||(groups[r]=[])).push(el);
  });
  Object.keys(groups).forEach(function(r){
    var list=groups[r];if(list.length<2)return;
    list.forEach(function(el,idx){
      el.classList.add('srSanctReady181');
      el.style.setProperty('--srReady',colorFor(r));
      var slot=Math.max(0,Number(el.dataset.sanctSlot)||0);
      el.style.setProperty('--srReadyDelay',(-((slot%4)*115+idx*35))+'ms');
    });
  });
}
function deferReadyPairs(){if(typeof queueMicrotask==='function')queueMicrotask(refreshReadyPairs);else Promise.resolve().then(refreshReadyPairs);}
...
function finishCleanup(){if(raf){cancelAnimationFrame(raf);raf=0;}document.documentElement.classList.remove('srSanctDragActive181');clearMarks();if(!drag){deferReadyPairs();return;}if(drag.source)drag.source.classList.remove('srSanctLift181');if(drag.ghost&&drag.ghost.parentNode)drag.ghost.remove();drag=null;deferReadyPairs();}
...
function promote(){if(!drag||drag.dragging)return;drag.dragging=true;document.documentElement.classList.add('srSanctDragActive181');drag.source.classList.add('srSanctLift181');drag.ghost=drag.source.cloneNode(true);drag.ghost.classList.remove('sanctDragging','srSanctLift181','srSanctReady181');drag.ghost.style.removeProperty('--srReady');drag.ghost.style.removeProperty('--srReadyDelay');drag.ghost.classList.add('srSanctGhost181');drag.ghost.removeAttribute('data-sanct-slot');document.body.appendChild(drag.ghost);markBoard(drag.rarity);updateTarget();schedule();}
...
/* Restore the pre-drag fusion anticipation: whenever Sanctuary renders, mark every
   mergeable duplicate group. This is presentation-only and leaves sanctMergeDrop
   as the sole movement/fusion authority. */
injectCss();
if(typeof scrSanctuaire==='function'){
  var baseScrSanctuaire181=scrSanctuaire;
  scrSanctuaire=function(){
    var out=baseScrSanctuaire181.apply(this,arguments);
    deferReadyPairs();
    return out;
  };
  try{if(typeof SCREENS!=='undefined'&&SCREENS&&SCREENS.sanctuaire)SCREENS.sanctuaire=scrSanctuaire;}catch(_){}
}
deferReadyPairs();
})();
