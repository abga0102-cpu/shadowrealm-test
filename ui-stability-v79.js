// HOME_LAYOUT_AND_MODAL_STABILITY_V79
// Conserve la file de modales V78, corrige les cibles tactiles réellement trop petites
// et autorise les fermetures clavier déclenchées explicitement par le joueur.
(function(){
  'use strict';

  const css=document.createElement('style');
  css.id='homeLayoutModalStabilityV79';
  css.textContent=`
    ...
    /* Cible tactile réelle >=44 px. La hauteur de l'accueil est récupérée sur les marges,
       pas en réduisant les boutons interactifs. */
    ...
    /* L'ancienne règle mobile appliquait scale(.9), ce qui ramenait une cible 44 px à 39,6 px.
       On garde désormais sa taille tactile réelle. */
    @media(max-height:700px){
      #screen:has(.campaignWorld) .worldAction,
      #screen:has(.campaignWorld) .worldMenu>summary{transform:none}
    }

    /* La croix des recommandations était explicitement forcée à 30 px dans style.css. */
  ...
  /* Si le moteur natif gère Échap pour fermer une fenêtre, cette fermeture est
     explicitement demandée par le joueur et ne doit pas être bloquée par V78/V79. */
