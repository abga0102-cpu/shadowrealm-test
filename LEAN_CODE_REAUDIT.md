...
The source-reference audit excluded ten active nested dependencies from the initial 39-file scan. They must not be source-retired. The corrected investigation list originally contained 29 files. Hero Equipment V1 and notification V105 then completed their staged unload/integration proof, reducing the list to 27. The Forge history audit retired V254/V258/V260/V261 after proving loaded V266 is the surviving batch-gate authority, reducing the list to 23. Rebirth V220 is now retired after V221 was verified as its explicit loaded replacement, leaving **22 investigation candidates**.
...
- Rebirth V220 audit: current `index.html` loads `rebirth-scroll-natural-v221.js`, not V220. V221 explicitly states that it replaces V220 behavior and deliberately sets the V220 installation sentinel to prevent the older layer from installing if encountered later. Commit `dcbc9035` introduced V221 as “V221 preserve natural Rebirth scroll like Familiar.”

## Remaining work and evidence required

| Phase / responsibility | Current evidence | Next reviewable scope and exit condition |
| --- | --- | --- |
| L1 / L5 historical source retirement | 22 root-level tracked JS files remain absent from the index and transitive Familiar loader paths after staged UI, Forge auto-batch history and Rebirth V220 retirements. Absence alone does not prove no other references or active work. | Audit references, archived workflows, tests and concurrent owner work per family; delete only proven obsolete sources and preserve active contracts. |
...
| L1/L5 Rebirth V220 scroll history — CANDIDATE | V220 is absent from runtime loaders. Loaded V221 explicitly says it replaces V220 behavior, sets `window.__srRebirthScrollStabilityV220=true` to suppress the older layer, and owns the live scroll-preservation object. | Retire V220 source, require V221 exactly once in the loader, require V220 source absence, preserve the V221 replacement/suppression contract, and pass the exact-head regression gate without changing V221 runtime behavior. |
...
The remaining investigation total is **22 files** after retiring Rebirth V220. The former staged UI candidates and four superseded Forge auto-batch gate sources are also no longer counted because their sources have completed retirement with surviving-owner evidence.
