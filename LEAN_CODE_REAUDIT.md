...
- PR #142: Forge panel history V259/V260/V261 and standalone entry-animation history V263/V264 completed source retirement after exact-head regression proof; loaded panel V266 and Forge UX V273 remain the canonical presentation owners.
...
- Later Forge UX audit: V261/V266/V268/V269/V270/V271/V272 form the superseded loot-presentation lineage behind loaded V273. The chain successively introduced consolidated card lifecycle, persistent kept stacks, hammer/anvil feedback, bounded queues and background safety; V273 retains those intended responsibilities while adding comparison-aware AUTO feedback and avoiding a permanent 140 ms watcher while idle. V253/V258 are deliberately excluded because they contain older compatibility/AUTO coupling that requires a separate audit.

## Remaining work and evidence required

| Phase / responsibility | Current evidence | Next reviewable scope and exit condition |
| --- | --- | --- |
| L1 / L5 historical source retirement | 10 root-level tracked JS investigation candidates remain if the current seven-file Forge UX-history retirement lands. Absence alone does not prove no other references or active work. | Audit references, archived workflows, tests and concurrent owner work per family; delete only proven obsolete sources and preserve active contracts. |
...
| L1/L5 later Forge UX history — CANDIDATE | V261/V266/V268/V269/V270/V271/V272 are unloaded, unreferenced historical Forge loot-presentation predecessors. Loaded V273 preserves the bounded/background-safe queue, current loot stack and animation lifecycle, adds comparison-aware AUTO feedback, and stops the 140 ms watcher whenever no fast work remains. | Retire the seven historical sources, keep V273 loaded and unchanged, enforce source absence + V273 lifecycle ownership in the existing Forge contract, and pass the exact-head regression gate. V253/V258 remain outside this batch. |
...
| --- | ---: | ---: | --- |
| Forge / equipment | 7 | 50457 | Feature-owner and archived-reference audit required; V253/V258 remain compatibility-sensitive |
...
The remaining investigation total is **10 files** if the current seven-file later Forge UX-history retirement lands. That batch removes **133,776 bytes** of already-unloaded source history; the normal-session runtime count remains 110 because none of those files are currently requested.
