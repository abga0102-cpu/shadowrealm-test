...
const APP_BUILD = document.querySelector('meta[name="shadowreach-build"]')?.content || "2026.09.06.50";
...
/* V379: abilities whose damage is not derived from enemy.dmg still need the
...
function enemyAbilityDamageV379(raw) {
  return Math.max(1, Math.floor(Math.max(0, Number(raw) || 0) * 0.60));
}
window.__srEnemyAbilityDamageV379 = enemyAbilityDamageV379;
...
const ELITE_ABIL = {
  ...
  cristal: {
    ...
    onLand(c, e) {
      ...
      const back = enemyAbilityDamageV379(Math.min(Math.floor(c.heroMaxHP * 0.30), Math.floor(e.stored * 0.6)));
      e.stored = 0;
      c.heroHP -= back;
      addShake(c, 6);
  ...
  eclats: {
    ...
    filter(c, e, dmg, src, crit) {
      ...
      const each = enemyAbilityDamageV379(c.heroMaxHP * 0.04);
      for (let k = 0; k < 3; k++) {
  ...
  pietinement: {
    ...
    tick(c, e, dt) {
      ...
      const hit = enemyAbilityDamageV379(c.heroMaxHP * 0.08);
      c.heroHP -= hit;
  ...
  epines: {
    ...
    filter(c, e, dmg, src, crit) {
      ...
      const back = enemyAbilityDamageV379(physical * 0.05);
      c.heroHP -= back;
  ...
  zoneBrulante: {
    ...
    tick(c, e, dt) {
      ...
      if (e._zone < 1) return;
      e._zone = 0;
      const b = enemyAbilityDamageV379(c.heroMaxHP * 0.02);
      c.heroHP -= b;
  ...
  souffle: {
    ...
    tick(c, e, dt) {
      if (e.breathWind > 0) {
        ...
        const d = enemyAbilityDamageV379(raw * (1 - heroDmgRed(c) / 100));
        c.heroHP -= d;
        c.heroHit = 0.3;
        addShake(c, 11);
  ...
  sismique: {
    ...
    tick(c, e, dt) {
      if (e.quakeWind > 0) {
        ...
        const d = enemyAbilityDamageV379(c.heroMaxHP * 0.25 * (1 - heroDmgRed(c) / 100));
        c.heroHP -= d;
        c.heroHit = 0.3;
        c.heroX = 24;
        c.heroStun = 2;
  ...
  souffleLave: {
    ...
    tick(c, e, dt) {
      if (e.channel > 0) {
        ...
        if (e._ch < 0.5) return;
        e._ch = 0;
        const d = enemyAbilityDamageV379(c.heroMaxHP * 0.07 * (1 - heroDmgRed(c) / 100));
        c.heroHP -= d;
        c.heroHit = 0.2;
  ...
  orbeInfernal: {
    ...
    tick(c, e, dt) {
      if (e.orbWind > 0) {
        ...
        const d = enemyAbilityDamageV379(c.heroMaxHP * (0.05 + 0.25 * near) * (1 - heroDmgRed(c) / 100));
        c.heroHP -= d;
        c.heroHit = 0.3;
        addShake(c, 10);
        addBurst(c, "crit", c.heroX, "#FF5A5A");
  ...
  corruption: {
    ...
    tick(c, e, dt) {
      if (c.petCorrupt > 0) {
        ...
        const d = enemyAbilityDamageV379(c.heroMaxHP * 0.03);
        c.heroHP -= d;
  ...
  bond: {
    ...
    tick(c, e, dt) {
      if (e.leapWind > 0) {
        ...
        const d = enemyAbilityDamageV379(c.heroMaxHP * 0.18 * (1 - heroDmgRed(c) / 100));
        c.heroHP -= d;
        c.heroHit = 0.3;
        addShake(c, 10);
        addBurst(c, "crit", e.x, "#B15CF6");
  ...
  mutation: {
    ...
    filter(c, e, dmg, src) {
      ...
      if ((e.mutations || 0) >= 3 && dmg > 0 && src !== "dot") {
        const back = enemyAbilityDamageV379(dmg * 0.3);
        c.heroHP -= back;
  ...
  etreinte: {
    ...
    tick(c, e, dt) {
      if (e.grip > 0) {
        ...
        e._grip = 0;
        const d = enemyAbilityDamageV379(c.heroMaxHP * 0.04 * (1 - heroDmgRed(c) / 100));
        c.heroHP -= d;
        c.heroHit = 0.2;
...
function tick() {
  ...
  if (c.burn > 0) {
    ...
    if (c._burnHero >= 1) {
      c._burnHero = 0;
      const b = enemyAbilityDamageV379(c.heroMaxHP * 0.005 * c.burn);
      c.heroHP -= b;
