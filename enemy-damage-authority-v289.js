/* SHADOWREACH V289 · Campaign enemy balance authority
   V351 extension: from Facile 5-11 (internal floor 91), Campaign enemy pressure
...
(function(){
  ...
  window.__srCampaignReferenceBalanceV351=true;
  ...
  function campaignBalanceFloorV351(f){
    var max=maxFloor();
  ...
  function campaignEnemyHP(f){
    ...
    return Math.max(1,Math.round(expectedDamage(campaignBalanceFloorV351(f))*TARGET_HITS_TO_KILL));
  }
  function campaignEnemyDamage(f){
    if(isIntroFloor(f))return INTRO_FLOOR_DAMAGE;
    return Math.max(1,Math.round(expectedHP(campaignBalanceFloorV351(f))/TARGET_HITS_TO_DEFEAT_REFERENCE));
  }
  ...
  window.__srV351CampaignBalanceFloor=campaignBalanceFloorV351;
  ...
  window.__srEnemyDamageConfigV289={
    version:351,maxFloor:800,legacyMaxFloor:400,semanticLegacyFloor:semanticLegacyFloor,
    ...
    campaign5_11Balance:{startFloor:CAMPAIGN_BALANCE_START,startStage:'5-11',minimumFloor:CAMPAIGN_BALANCE_MIN,minimumStage:'5-8',offset:CAMPAIGN_BALANCE_OFFSET,balanceFloor:campaignBalanceFloorV351},
