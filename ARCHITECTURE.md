...
| --- | --- | --- |
| Bottom navigation / permanent navigation taxonomy / hub composition / render lifecycle | `bottom-nav-layout-v183.js` | Despite the historical filename, this file contains the V209 BottomNav authority. It owns fantasy icon markup/decoration, canonical geometry, the displayed Home / Equipment / Progression / Menu taxonomy while preserving the legacy `developpement` / `parametres` route IDs, duplicate-free Progression/Menu hub composition, the sole `renderTabs` lifecycle wrapper, and post-render `sr:bottomnavrendered`. It also owns navigation-level suppression of redundant permanent Home shortcuts; contextual Home recommendations remain feature-owned. |
| Premium interaction / BottomNav visual polish / semantic action styling | `premium-ui-v209.js` | Visual/material styling only. Owns the final no-badge/no-halo recommendation presentation plus the shared CTA hierarchy: gold primary actions, navy/steel secondary actions, red danger actions and muted disabled state. Jewel colors remain available for domain/status presentation. Not the BottomNav geometry or render-lifecycle owner. |
...
| Accomplishments modal / title rendering + title interaction / Progression entry | `accomplishments-canonical-v139.js` | Canonical `ACT.accomplishments` renderer, title owner and Progression-screen Accomplishments entry owner. It opens the canonical modal directly and must not wrap the global `openModal` function. |
...
| Floating Social launcher policy | `social-v1.js` | Owns launcher creation/remount and suppression policy when Social is enabled. Social remains conditional and is not exposed as a permanent Menu route unless its runtime owner supplies a valid route contract. |
