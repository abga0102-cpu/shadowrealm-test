...
| Modal stability / modal-state publication / campaign compact tagging | `ui-stability-v83.js` | Owns the guarded modal open/close queue, overlay persistence tagging and canonical `sr:modal-state` publication. Its remaining `#app` child-list observer is modal-only fallback synchronization for those duties. Campaign compact tagging (`srHomeCompact`) is no longer observer-driven: it follows canonical `sr:bottomnavrendered` with one animation-frame deferral plus startup sync. |
