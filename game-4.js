...
function scrAccueil() {
  ...
  return recommendedCard + '<div class="campaignWorld">' +
    ...
    '<div class="pad mt4"><div class="card frame homeForge" id="homeForge">' +
      '<div class="fgRow forgeHeaderRow">' +
        ...
        '<button type="button" class="iBtn iconBtn" data-act="rarityInfo" title="Raretés" aria-label="Informations sur les raretés">i</button>' +
...
function scrEquipement() {
  ...
  return topbar("Équipement", '<span class="pill">'+S.inventory.length+' objets</span>') + '<div class="pad mt6">' +
    ...
    '<div class="sect" style="margin:14px 0 8px">Équipements stockés</div><div class="equipFilters" role="group" aria-label="Filtrer les équipements">'+filters.map((f)=>'<button type="button" class="'+(invFilter===f?'on':'')+'" data-act="invFilter" data-arg="'+f+'" aria-pressed="'+(invFilter===f?'true':'false')+'">'+(f==='ALL'?'Tout':SLOT_LABEL[f])+'</button>').join('')+'</div>' +
    '<div class="equipInventoryActions mt6">'+btn(recycleSelectMode?'Annuler sélection':'Sélection manuelle',{small:true,cls:recycleSelectMode?'blue':'ghost',act:'toggleRecycleSelect'})+btn('Recycler catégories',{small:true,cls:'dark',act:'recycleSlotsPicker'})+'</div>' +
