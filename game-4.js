...
function scrSanctuaire() {
  ...
  if(!unlocked) return topbar("Sanctuaire",resPills)+
    ...
    '<div class="dim small mt8" style="line-height:1.55">Vaincs ton premier Méga-Boss pour réveiller le Sanctuaire.<br>La Fusion transforme des ressources existantes en accélérateurs, sans timer.</div></div></div>';
  ...
  const preview=recipe ? '<div class="notice mt8 tiny"><b style="color:var(--goldLit)">'+esc(recipe.name)+'</b> · coût : '+
      Object.keys(SANCT_ING).filter(k=>sanctNeedFor(recipe,k)).map(k=>sanctNeedFor(recipe,k)+' '+SANCT_ING[k].label).join(' + ')+
      ' → <b>'+(recipe.out==='seal' ? recipe.qty+'× Sceau de stabilité' : recipe.qty+'× Accél. '+esc((ACCEL_DEFS.find(a=>a.key===recipe.out)||{}).label||recipe.out))+'</b></div>'
    : (st.slotA&&st.slotB?'<div class="notice mt8 tiny">Combinaison inconnue. Tu peux tenter la Fusion sans perdre de ressource si aucune recette ne correspond.</div>':'');
  const known=SANCT_RECIPES.map((r)=>{
    const d=!!st.discovered[r.id];
    if(!d && r.secret) return '<div class="card mt6" style="padding:7px 9px"><div class="b small">❓ Recette secrète</div><div class="mute tiny">Découvre la bonne combinaison.</div></div>';
    return '<div class="card mt6" style="padding:7px 9px"><div class="between"><div><div class="b small">'+(d?'✓ ':'')+esc(r.name)+'</div><div class="mute tiny">'+
      (d ? r.aq+' '+SANCT_ING[r.a].label+' + '+r.bq+' '+SANCT_ING[r.b].label : 'Recette non découverte')+'</div></div><span class="pill">'+
      (d ? (r.out==='seal' ? r.qty+'× Sceau de stabilité' : r.qty+'× '+esc((ACCEL_DEFS.find(a=>a.key===r.out)||{}).label||r.out)) : '???')+'</span></div></div>';
  }).join('');
  return topbar("Sanctuaire",'<span class="pill" style="color:var(--goldLit);border-color:var(--goldDim)">⏱️ '+fmtTime(accelTotal*60)+'</span><span class="pill">🛡️ '+fmt(st.stabilitySeals||0)+'</span>')+
    '<div class="pad mt6"><div class="card frame"><div class="between"><div><div class="bb gt">CERCLE DE FUSION</div><div class="mute tiny b">Fusion instantanée · aucun timer</div></div><span class="pill">'+fmt(st.fusions||0)+' fusions</span></div>'+
    '<div class="row gap8 mt10">'+slot(st.slotA,'a')+'<div class="bb gt">＋</div>'+slot(st.slotB,'b')+'</div>'+preview+
    '<div class="mt8">'+btn(ic("flame",14)+' FUSIONNER',{cls:recipe?'purple':'dark',act:'sanctFuse',dis:!(st.slotA&&st.slotB)})+'</div></div>'+
    '<div class="sect">Ressources</div><div class="row gap6" style="flex-wrap:wrap">'+ingredients+'</div>'+
    '<div class="sect">Livre de Fusion</div>'+known+
    '<div class="notice mt8 tiny">Les recettes découvertes restent inscrites dans le Livre. Une combinaison invalide ne consomme rien pendant cette version de test.</div></div>';
}
