...
#srAchArenaLauncher138 .srAchLaunchProgress{display:block;margin-top:3px;color:#9fb0c8;font-size:7px;letter-spacing:0;white-space:nowrap}\
...
function placeArenaLauncher(){
 try{
  ...
  var st=launcherState()||{},done=Math.max(0,Number(st.done)||0),total=Math.max(1,Number(st.total)||10),claimable=Math.max(0,Number(st.claimable)||0),stage=String(st.stage||'');
  b.innerHTML='<span class="srAchLaunchMark">✦</span><span class="srAchLaunchLabel">PASS</span><span class="srAchLaunchProgress">'+(stage?stage+' · ':'')+done+'/'+total+'</span>'+(claimable?'<span class="srAchLaunchBadge">'+(claimable>9?'9+':claimable)+'</span>':'');
 }catch(_){}
}
var q=false,retry=0;
