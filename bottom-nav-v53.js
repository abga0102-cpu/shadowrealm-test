// BOTTOM_NAV_FANTASY_V65
// Navigation principale fantasy, rétablie après la simplification du HUD.
(function(){
  const ICONS={
  ...
  function apply(){
    ...
    root.querySelectorAll('button,.tab,[data-act]').forEach(el=>{
      ...
      const old=el.querySelector('.fantasyNavIcon,svg,img');
      if(old&&old.classList&&old.classList.contains('fantasyNavIcon'))return;
      if(old)old.outerHTML=ICONS[k];else el.insertAdjacentHTML('afterbegin',ICONS[k]);
      el.classList.add('fantasyNavV65');el.dataset.fantasyNav='65';
    });
  }
  const css=document.createElement('style');css.id='fantasyNavStyleV65';css.textContent=`
    ...
    #tabs .fantasyNavV65:not(.on):not(.active):not([aria-current="page"]) .fantasyNavIcon{opacity:.74;filter:saturate(.78) brightness(.9) drop-shadow(0 2px 3px #0009)}
