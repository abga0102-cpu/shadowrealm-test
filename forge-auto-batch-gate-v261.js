/* SHADOWREACH · Forge Auto-Batch Gate V261
   Final progression authority. No dynamic loaders.
   Must load after forge-ux-v261.js and forge-panel-authority-v261.js.
*/
(function(){
'use strict';
if(window.__srForgeBatchGateV261)return;window.__srForgeBatchGateV261=true;
...
if(typeof ACT!=='undefined'&&ACT){
  var base=ACT.autoForgeBatch261;
  ACT.autoForgeBatch261=function(a){
...
function refresh(){
  ...
  document.querySelectorAll('#srAutoBatch261 .srBatch261').forEach(function(b){
    var n=Math.floor(Number(b.getAttribute('data-arg'))||1),locked=n>max;
    b.disabled=locked;b.classList.toggle('srBatchLocked261',locked);b.classList.toggle('on',!locked&&n===safeChoice(S.forge.autoBatch));
    if(locked){b.setAttribute('aria-disabled','true');b.textContent='🔒 ×'+n;}else{b.removeAttribute('aria-disabled');b.textContent='×'+n;}
  });
  var box=document.getElementById('srAutoBatch261');if(box){var note=box.querySelector('.srBatchGateNote261');if(!note){note=document.createElement('div');note.className='srBatchGateNote261 mute tiny mt3';box.appendChild(note);}note.textContent='Maximum actuellement débloqué : ×'+max+'.';}
}
...
var st=document.createElement('style');st.id='srForgeBatchGate261Style';st.textContent='.srBatch261.srBatchLocked261{opacity:.34!important;filter:grayscale(1);cursor:not-allowed!important;border-color:#354258!important;color:#738099!important;background:#0a111d!important;box-shadow:none!important}.srBatch261.srBatchLocked261:active{transform:none!important}';document.head.appendChild(st);
setTimeout(refresh,0);
window.__srForgeBatchGateV261={unlocked:unlocked,sanitize:sanitize,refresh:refresh,version:261};
})();