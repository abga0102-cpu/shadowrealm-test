...
test('V439 owns permanent Defense without adding a save field or a parallel runtime module', async ({}, testInfo) => {
  ...
  expect(index).toContain('shadowreach-build" content="2026.09.24.439"');
  expect(index).toContain('game-1.js?v=2026.09.24.439a');
  expect(index).toContain('game-2.js?v=2026.09.24.439b');
  expect(index).toContain('game-4.js?v=2026.09.24.439c');
  expect(index).toContain('game-5.js?v=2026.09.24.439d');
  expect(index).toContain('forge-comparison-authority-v146.js?v=2026.09.24.439e');
});

test('V439 derives Defense from legacy-compatible HP gear and caps permanent reduction at 70%', async ({ page }) => {
...
test('V439 Defense is multiplicative after existing temporary reduction and displayed in Equipment UI', async ({ page }) => {
  await openCleanGame(page);
  const result = await page.evaluate(() => {
    const originalD = D;
