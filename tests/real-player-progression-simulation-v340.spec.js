...
test('V340 models player power from actually attainable progression instead of maxed profiles', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    function equipRealisticCrafts(st, craftCount, seed) {
      ...
      for (let i = 0; i < craftCount; i += 1) {
        ...
        item.affixes = []; // random affixes are upside, not baseline progression.
    ...
    function simulatePet(st, summonCount, seed) {
      ...
      // Deliberately conservative: direct summons only. Fusion can only improve
      // this result, so the simulation never invents a high-rarity Familiar.
      if (summonCount > 0) {
    ...
    const profiles = {
      ...
      normal: {
        // 12 Raid Or clears by floor 75. Even spending every available Gold on
        // Forge keeps this reference below Forge 15, matching real play feedback.
  ...
  // The old V313 reference profiles used level 100 and Forge 35/50 in the
  // early semantic runway. A resource-earned fresh-save model must not silently
  // recreate those maxed assumptions.
