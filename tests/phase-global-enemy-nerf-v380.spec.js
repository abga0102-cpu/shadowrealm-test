...
test('V380 globally reduces enemy HP and damage and keeps Mega-Boss ratios', async ({ page }) => {
  await page.goto('/index.html?smoke=1');
  await page.waitForFunction(() => window.__srGlobalEnemyNerfV380 && window.__srEnemyDamageConfigV289);
  ...
  expect(cfg.campaignHpMulV380).toBeCloseTo(0.54, 8);
  ...
  expect(cfg.globalEnemyNerfV380.appliesTo).toEqual(
  ...
  expect(source).toContain('GLOBAL_HP_MUL_V380=0.90');
  expect(source).toContain('GLOBAL_DAMAGE_MUL_V380=0.60');

  const index = fs.readFileSync(path.join(process.cwd(), 'index.html'), 'utf8');
  expect(index).toContain('enemy-damage-authority-v289.js?v=2026.09.18.380');
  expect(index).toContain('game-2.js?v=2026.09.18.380');
});
