    ...
    window.MutationObserver = class extends NativeMO {
      constructor(cb) {
        ...
        const source = /forge-worn-details-v145/.test(stack) ? 'forge-worn-details-v145' :
          ...
          /auto-forge-v103/.test(stack) ? 'auto-forge-v103' :
        ...
        super((records, obs) => { rec.calls += 1; rec.records += records.length; return cb(records, obs); });
...
async function spamRealForge(page, mineral, ms) {
  ...
  return page.evaluate(async (ms) => {
    ...
    while (performance.now() - started < ms) {
      ...
      attempts++;
      if (b) { liveButtons++; if (!b.disabled) enabledButtons++; b.click(); }
...
async function snapshot(page) {
  return page.evaluate(() => ({
    ...
    ux: window.__srForgeUXV273 ? { kept:window.__srForgeUXV273.kept().length, pending:window.__srForgeUXV273.pending(), suspended:window.__srForgeUXV273.suspended(), watching:window.__srForgeUXV273.watching() } : null,
    observers: (window.__forgeDiag?.observers || []).map(x => ({ source:x.source, calls:x.calls, records:x.records, disabled:x.disabled })),
    errors: (window.__forgeDiag?.errors || []).slice(), bootErrors:(window.__srBootErrors || []).slice()
  }));
}
...
test.only('diagnostic: rich Forge stress with V151 observer disabled', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium-desktop', 'Chromium root-cause bisection');
  test.setTimeout(25000);
  await openInstrumentedGame(page, ['forge-equipment-safety-v151']);
  ...
  console.log('FORGE_DIAG_NO_V151', JSON.stringify(result));
  expect(after.errors).toEqual([]); expect(after.bootErrors).toEqual([]);
  throw new Error('FORGE_DIAG_NO_V151 ' + JSON.stringify(result));
});
