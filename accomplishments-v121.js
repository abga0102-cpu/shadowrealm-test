/* Shadowreach v121 - Accomplissements retroactifs
   Compatibility-only runtime shell: normalize historical accomplishment state
   and preserve the legacy raid/familiar progress signals still consumed by the
   canonical V139/V140 accomplishment owners. */
(function(){
  'use strict';
  ...
  /* Initialize/migrate accomplishment state once at module startup. The state is
     also normalized at every tracked event entry point, so V121 does not need
     UI, claim, reward-table or global render ownership. */
  try{if(typeof S!=='undefined'&&S)ensure(S);}catch(_){}

  /* A raid victory is recorded exactly when its result is presented. This keeps
     the original result-time semantics without a permanent observer. */
  ...
  /* Preserve the historical highest fused familiar rank used by old saves and
     compatibility paths. Canonical accomplishment UI/claims no longer live here. */
  if(typeof ACT!=='undefined'&&ACT&&typeof ACT.fuse==='function'){
    const oldFuse=ACT.fuse;
    ACT.fuse=(a)=>{
      const before=(S.pets||[]).map(p=>p.id);
      oldFuse(a);
      const after=(S.pets||[]).filter(p=>before.indexOf(p.id)<0);
      if(after.length){
        const best=Math.max.apply(null,after.map(p=>RANK[p.rarity]??-1));
        if(best>=0)update(s=>{const x=ensure(s);x.fusedPetRank=Math.max(x.fusedPetRank,best);});
      }
    };
  }
})();
