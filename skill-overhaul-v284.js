/* SHADOWREACH V284 / V457 · Intrinsic skill progression
   Skills use their own fixed power, duplicate gauge and +20%/level growth.
   Duplicate requirements follow 1/2/3/4/5/6 and stay capped at 6.
...
function growth(level){return 1+Math.max(0,(Number(level)||1)-1)*.20;}
...
window.__srV284DupesNeeded=function(level){level=Math.max(1,Math.floor(Number(level)||1));return Math.min(6,level);};
